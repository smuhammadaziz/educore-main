...
function EditCourseTeacher() {
  ...
  useEffect(() => {
    async function fetchCourse() {
      try {
        ...
        setDescription(data.Data.descr || '');
        setMain(data.Data.subject || '');
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (photo) {
      ...
      const response = await fetch(
        'https://st2.depositphotos.com/1350793/8441/i/450/depositphotos_84416316-stock-photo-hand-pointing-to-online-course.jpg',
      );
    ...
    try {
      const response = await fetch(`${backurl}api/update/course/${course_id}`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  return (
    <DefaultLayoutTeacher>
      ...
      <div className="mb-5 text-left mx-auto text-2xl">
        Editing <span className="underline">Course</span>
      </div>
